...
class ByPeriodDvbProgram extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      statType: 'day',
  ...
  renderRange () {
    ...
    const tag = false
    const props2 = {start, end, startSuffix, endSuffix, tag, format: SHOW_FORMAT[statType]}
    const mainChange = {
      onStartChange: start =>this.handleOnStartChange(start) ,
    ...
    switch (statType) {
      case 'day':
        return <ByDay {...props2} {...mainChange}  />
      case 'halfmonth':
  ...
  handleOnStartChange (value) {
    this.setState({start:value})
    if (this.props.onStartTimeChange){
      const startTime = value.format(DATE_FORMAT['day'])
