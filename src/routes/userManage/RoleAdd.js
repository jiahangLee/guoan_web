...
class DynamicRole extends React.PureComponent {
  ...
  showModelHandler = (e) => {
    ...
    if(!userAuth.checkUserAuth.getUserAuth(userAuth.ROLEADD)){
      this.state = {
        visible: false,
      };
      Modal.error({
  ...
  componentWillMount () {
    ...
    /**
    const data = fetch(REST_USER_API.AREATREE , {
      method: 'POST',
      mode: 'cors',
      body: JSON.stringify(),
      headers: {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        const rolenama = values.roleName;
        if(rolenama==null || rolenama=='' || rolenama==undefined){
          Modal.warning({
            title: '请输入角色名称',
            okText: '确定'
          });
        ...
        const operatorId = session.get("operatorId");
        let param = '?operatorId='+operatorId+'&roleName='+ values.roleName + '&areaList=' + values.areaList + '&funcList=' +values.funcList;
        const data = fetch(REST_USER_API.ROLEADD + param, {
          method: 'POST',
          mode: 'cors',
          body: JSON.stringify(),
          headers: {
          ...
          <div style={{display: 'flex',  flexFlow:'row nowrap'}}>
            <div>
              {getFieldDecorator('areaList', {Roles: [{required: false}]})(
                <CommonTree ref={self => this.areaTree = self} title='区域权限' dataChecked={[]} data={areaTrees}/>
              )}
            </div>
            <div>
              {getFieldDecorator('funcList', {Roles: [{required: false}]})(
                <CommonTree  ref={self => this.funcTree = self} title='功能权限' dataChecked={[]} data={funcTrees}/>
              )}
            </div>
          </div>
        </FormItem>
