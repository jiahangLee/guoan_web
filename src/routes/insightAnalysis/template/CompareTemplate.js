...
class CompareTemplate extends React.PureComponent {
  constructor (props) {
    super(props)
    this.state = {
  ...
  onPeriodChange=v=>{
    ...
    if(biz=='all' && bizSubtype == 'useDays' && v == 'day')return
  ...
  render () {
    const {common, loading, labelField, title, columnArr, compareTypes, rowKeyField, legendField,biz,bizSubtype} = this.props
    ...
    const chartConfig = {labelField, legendField, legends: compareTypes}
...
CompareTemplate.propTypes = {
  ...
  compareTypes: React.PropTypes.array.isRequired,
}
...
export default connect(({common, loading}) => ({common, loading: loading.models.common}))(CompareTemplate)
