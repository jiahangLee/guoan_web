...
const KPI_GROUP = [
  ...
  , {
    label: '流动类',
    value: [KPIS.userFlowRate,KPIS.flowInUser,KPIS.flowOutUser]
  }
  , {
    label: '用户指数类',
...
const COLUMN_ARRAY = [COLUMNS.area_name, COLUMNS.statistic_time, COLUMNS.game_visit_single_game_name]
const TITLE = '单游戏用户分析'
const BIZ = 'game_visit'
const BIZ_SUBTYPE = 'single_game'

const BusinessGameType1Analysis = () => {
  return (
    <ManyCompareTemplate title={TITLE} kpiGroup={KPI_GROUP} biz={BIZ} bizSubtype={BIZ_SUBTYPE}
                         legendField={COLUMNS.game_visit_single_game_name.en} columnArr={COLUMN_ARRAY}
                         rowKeyField={COLUMNS.statistic_time.en + COLUMNS.game_visit_single_game_name.en}/>
  )
}

export default BusinessGameType1Analysis
