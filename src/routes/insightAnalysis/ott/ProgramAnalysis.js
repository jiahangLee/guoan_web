...
const KPI_GROUP = [
  {
    ...
    label: '使用次数类',
    value: [KPIS.useCount, KPIS.useCountAvg,KPIS.useCountAvgOnline,KPIS.userUseCountAvg,KPIS.userUseCountAvgOnline,KPIS.bounces,KPIS.outOfProportion]
  }, {
    label: '使用天数类',
...
const COLUMN_ARRAY = [COLUMNS.area_name, COLUMNS.statistic_time, COLUMNS.program_type]
const TITLE = 'OTT点播节目分析'
const BIZ = 'ott'
const BIZ_SUBTYPE = 'program'

const ProgramAnalysis = () => {
  return (
    <ManyCompareTemplate title={TITLE} kpiGroup={KPI_GROUP} biz={BIZ} bizSubtype={BIZ_SUBTYPE} legendField={COLUMNS.program_type.en} columnArr={COLUMN_ARRAY} rowKeyField={COLUMNS.statistic_time.en+COLUMNS.program_type.en} />
  )
}
